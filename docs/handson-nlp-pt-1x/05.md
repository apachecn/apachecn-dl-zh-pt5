<title>B12365_03_Final_JC_ePub</title>
...
我们可以下载一组预先计算的单词嵌入来演示它们是如何工作的。为此，我们将使用**全局向量进行单词表示** ( **手套**)嵌入，可以从这里下载:[https://nlp.stanford.edu/projects/glove/](https://nlp.stanford.edu/projects/glove/)。这些嵌入在非常大的 NLP 数据语料库上计算，并在单词共现矩阵上训练。这是基于一个概念，即出现在一起的单词更有可能有相似的意思。例如，单词*孙*可能更频繁地与单词*热*一起出现，而不是与单词*冷*一起出现，因此更有可能的是，*孙*和*热*被认为更相似。
