...
1.  Import all the necessary libraries:
    ...
    首先，需要导入 TensorFlow 库。您将使用`tensorflow_datasets`加载数据集，`tensorflow.keras.models`构建顺序张量流模型，`tensorflow.keras.layers`向 CNN 模型添加层，`RMSprop`作为优化器，`matplotlib.pyplot`和`matplotlib.image`用于一些快速可视化。
