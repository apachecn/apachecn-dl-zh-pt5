...
3.  创建一个张量流函数，将两个张量相乘，并使用`ones_like`函数将值`1`添加到结果张量中的所有元素，以创建一个与矩阵乘法结果形状相同的张量。然后，对张量的每个值应用 sigmoid 函数:
...
## 活动 3.02:从 TensorFlow Hub 的预训练模型中执行单词嵌入
...
<title>B16341_Authors_Page_ePub</title>![Rayon](img/Matthew_Moocarme.png)
...
我们是这本书的作者马修·穆卡米、安东尼·索和安东尼·马德兰。我们真的希望你喜欢阅读我们的书，并发现它对学习张量流有用。
