...
现在，我们将定义与TensorFlow相关的变量和运算。在此之前，我们将创建一个名为`models`的目录，用于存储变量:
...
此后，我们将创建一个`tf.train.Saver`对象。`Saver`对象可用于将TensorFlow变量保存到内存中，以便日后需要时可以恢复。在下面的代码中，我们将把嵌入变量保存到名为`model.ckpt`的`models`目录中:
