...
这里， *f* 表示用于计算*y[I+1]和*s[I]的实际更新规则。利用注意力机制，我们为第*I^(th)解码步骤引入了新的时间相关上下文向量*c[I]。*c[I]T21 矢量是所有展开的编码器步骤的隐藏状态的加权平均值。如果第*j^(th)th*字对于翻译目标语言中的第*I^(th)th*字更重要，则将给予编码器的第 th*隐藏状态更高的权重。现在解码器 LSTM 变成了这样:****
