...
*   **目标函数**:目标函数可以是最小化或最大化给定超参数配置的一些度量值。例如，在 DL 模型训练和微调场景中，我们希望最大化 NLP 文本分类器准确度的 F1 值。这个目标函数需要包装成一个可训练的函数，其中射线调谐可以做 HPO。在下一节中，我们将说明如何包装我们的 NLP 文本情感模型。
...
*   *自动超参数调谐*:[https://in said . medium . com/Automated-Hyperparameter-tuning-988 b5 aeb 7 F2 a](https://insaid.medium.com/automated-hyperparameter-tuning-988b5aeb7f2a)
