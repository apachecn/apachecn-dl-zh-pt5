<title>B15778_06_Final_SZ_ePub</title>

# 6。用 RNNs 分析数据序列

概观

本章详述了递归神经网络的概念。你将了解到**递归神经网络** ( **RNNs** )的学习过程，以及它们如何存储记忆。本章将介绍**长短期记忆** ( **LSTM** )网络架构，它利用短期和长期记忆，通过数据序列来解决数据问题。在本章结束时，你将牢固掌握 RNNs 以及如何解决自然语言处理数据问题。
...
为情感分析执行 NLP 的过程将在下一节详细解释。我们将解释单词嵌入的概念以及在 PyTorch 中开发这样一个模型可以执行的不同步骤，这将是本章最后一个活动的目标。
