...
启动一个TensorFlow`Session`并初始化所有变量:
...
胶囊是一组学习检测图像中特定特征的神经元；说，眼睛。与返回标量的神经元不同，胶囊返回矢量。向量的长度告诉我们特定的特征是否存在于给定的位置，并且向量的元素表示特征的属性，例如位置、角度等等。
