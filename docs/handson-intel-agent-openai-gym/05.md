...
在山地汽车Gym环境中，汽车位于两座山之间的一维轨道上。目标是把车开上右边的山；然而，这辆车的发动机不够强劲，即使以最大速度行驶也无法上山。所以，成功的唯一方法就是来回开车造势。简而言之，山地车问题就是让一辆动力不足的车到达山顶。

在你实现你的智能体算法之前，理解环境、问题、状态和动作空间会有很大的帮助。我们如何在Gym里找出山地车环境的状态和动作空间？嗯，我们已经从第 4 章、*探索Gym及其特点*中知道了如何去做。我们编写了一个名为`get_observation_action_space.py`的脚本，它将打印出环境的状态、观察和动作空间，其名称作为第一个参数传递给脚本。让我们用下面的命令让它打印出`MountainCar-v0`环境的空格:

```
(rl_gym_book) praveen@ubuntu:~/rl_gym_book/ch4$ python get_observation_action_space.py 'MountainCar-v0'
```

注意，命令提示符有`rl_gym_book`前缀，这表示我们已经激活了`rl_gym_book` conda Python 虚拟环境。此外，当前目录`~/rl_gym_book/ch4`表示脚本是从与本书代码库中的[第 4 章](part0071.html#23MNU0-22c7fc7f93b64d07be225c00ead6ce12)、*探索Gym及其特性*的代码对应的`ch4`目录运行的。
...
如果你想重温一下**框**和**离散的**空间的含义，你可以快速翻到[第 4 章](part0071.html#23MNU0-22c7fc7f93b64d07be225c00ead6ce12)、*探索Gym及其特点*，我们在Gym 部分的*空间下讨论了这些空间及其含义。理解它们很重要。*

下表总结了状态和动作空间类型、描述以及允许值的范围，供您参考:

| **MountainCar-v0 环境** | **类型** | **描述** | **范围** |
| 状态向量空间 | `Box(2,)` | (位置、速度) | 位置:-1.2 到 0.6速度:-0.07 到 0.07 |
...
让我们重温一下我们在[第 4 章](part0071.html#23MNU0-22c7fc7f93b64d07be225c00ead6ce12)、*探索Gym及其功能*中使用的强化学习Gym锅炉板代码，如下所示:
...
所以，我们已经准备好了智能体。你可能会问，下一步我们该怎么办。嗯，我们应该在Gym环境中训练智能体！在下一节中，我们将了解培训程序。
...
一旦我们让智能体在Gym训练，我们希望能够衡量它学得有多好。为了做到这一点，我们让智能体通过一个测试。就像在学校一样！`test(agent, env, policy)`采用智能体对象、环境实例和智能体策略来测试环境中智能体的性能，并返回一个完整剧集的总报酬。它类似于我们前面看到的`train(agent, env)`函数，但是它不允许智能体学习或更新它的 Q 值估计:
...
在最初的培训集中，当智能体刚刚开始学习时，你会看到它总是以 *-200* 的奖励结束。从您对Gym的山地车环境如何工作的理解中，您可以看到智能体没有在 *200* 时间步长内到达山顶，因此环境会自动重置环境；这样，智能体只能得到 *-200* 。还可以观察 **![](img/00127.jpeg)** ( **eps** )探索值慢慢衰减。
...
我们从了解著名的山地车问题开始，看看在Gym的`MountainCar-v0`环境中，环境、观察空间、状态空间和奖励是如何设计的。我们重温了在上一章中使用的强化学习Gym样板代码，并对其进行了一些改进，这些也可以在本书的代码库中找到。
...
我希望您在实现智能体和观看它在Gym解决山地汽车问题的过程中获得了很多乐趣！我们将在下一章探讨解决各种更具挑战性问题的高级方法。