...
通过在任何Gym环境中运行`parameters.json`文件中的`learning_step_thresh`参数设置为 1(对于 1 步返回)和 5 或 10(对于 n 步返回)的`ch8/a2c_agent.py`脚本，您可以看到具有 1 步返回的 advantage 演员评论家智能体的性能与具有 n 步返回的性能相比如何。
...
我们现在已经拥有了完成本章目标所需的所有部件，即组装一个智能的自动驾驶智能体，然后训练它在照片般逼真的卡拉驾驶环境中自动驾驶汽车，这是我们在前一章中使用Gym界面开发的学习环境。智能体培训过程可能需要一段时间。根据您要培训智能体的机器的硬件，这可能需要几个小时的时间(对于较简单的环境，如`Pendulum-v0`、`CartPole-v0`和一些 Atari 游戏)到几天的时间(对于复杂的环境，如 CARLA 驾驶环境)。为了首先很好地理解培训过程以及如何在智能体培训时监控进度，我们将从几个简单的示例开始，来浏览培训和测试智能体的整个过程。然后，我们将看看如何轻松地将它转移到 CARLA 驾驶环境中，以进一步训练它。



# 训练和测试深层 n 步优势演员评论家智能体

因为我们的智能体的实现是通用的(如前一节步骤 1 中使用的表格所讨论的)，我们可以使用任何具有Gym兼容界面的学习环境来训练/测试智能体。您可以在本书前几章讨论的各种环境中试验和训练智能体，我们还将在下一章讨论一些更有趣的学习环境。别忘了我们定制的卡拉汽车驾驶环境！
...
您可以将`Pendulum-v0`替换为您机器上设置的任何与Gym兼容的学习环境名称。
...
让我们开始在卡拉驾驶环境中训练一个智能体吧！首先，确保你的 GitHub fork 与上游的 master 保持一致，这样你就可以从书的库中获得最新的代码。由于我们在上一章中创建的 CARLA 环境与 OpenAI Gym接口兼容，因此使用 CARLA 环境进行训练实际上很容易，就像任何其他Gym环境一样。您可以使用以下命令训练 n 步优势演员评论家智能体:
