...
| `_render()` | (可选)这会渲染Gym环境的状态或观察。 |
| `_close()` | (可选)这将关闭Gym环境。 |
| `_seed` | (可选)这为Gym环境中的随机函数播种了一个自定义种子，使环境的行为对于给定的种子来说是可重现的。 |
...
基于我们已经讨论过的Gym环境的剖析，我们现在将展示一个名为`CustomEnv`的定制环境类实现的基本版本，它将是`gym.Env`的子类，并实现使其成为Gym兼容环境所需的基本方法和参数。最小实现的模板如下:
...
# 创建一个开放的Gym兼容卡拉驾驶模拟器环境

CARLA 是一个建立在非真实引擎 4 游戏引擎之上的驾驶模拟器环境，与一些竞争对手相比，它的渲染更加真实。你可以在 https://carla.org 的官方网站上了解更多关于卡拉模拟器的信息。在本节中，我们将研究如何创建一个定制的 OpenAI Gym 兼容的汽车驾驶环境来培训我们的学习智能体。这是一个相当复杂的环境，需要一个 GPU 来运行——这与我们迄今为止看到的其他Gym环境不同。一旦您理解了如何为 CARLA 创建一个与Gym兼容的定制环境界面，您就有足够的信息来为您自己的定制环境开发界面，不管它们有多复杂。
...
在我们开始一个Gym兼容的卡拉环境之前，让我们先来看看卡拉模拟器。所以，让我们继续下载 CARLA 发布版的二进制文件。在下一节中，我们将使用`VER_NUM`来表示版本号，因此在运行以下命令之前，请确保将`VER_NUM`文本替换为您正在使用的版本号:
...
你可能已经注意到了，在每一集的开始，我们称之为Gym环境的`reset`方法。对于 CARLA 环境，我们希望通过 CARLA 客户端更新 CARLA 服务器以重启关卡。
...
我们已经实现了`meta hod`来计算奖励，并为定制的 CARLA 环境定义了允许的操作、观察和重置方法。根据我们的定制Gym环境创建模板，这些是我们需要实现的必需方法，用于创建与 OpenAI Gym接口兼容的定制环境。
...
虽然我们可以选择任何标准来重新设置卡拉Gym环境，但有三点需要考虑，如下所示:

*   当智能体控制的宿主或自我汽车与汽车、行人、建筑物或其他路边物体发生碰撞时，这可能是致命的(类似于在 Atari 游戏中丧生)
*   当宿主或自我车到达其目的地或最终目标时
*   当超过时间限制时(类似于我们在山地汽车Gym环境中的 200 时间步长限制)
...
现在，我们已经完成了创建基于 CARLA 驾驶模拟器的自定义Gym兼容环境所需的所有组件！在下一节中，我们将测试环境并最终看到它的运行。
...
所以，你现在有你的自定义卡拉Gym环境工作！您可以使用`ch7/carla-gym/carla_gym/envs/scenarios.json`文件中的定义创建几种不同的驾驶场景。然后，您可以为这些场景中的每一个创建新的定制 CARLA 环境，在您注册了定制环境之后，您可以使用常规的`gym.make(...)`命令，例如`gym.make("Carla-v0")`。

书中代码库中的代码使用我们在本章前面讨论过的方法来处理 OpenAI Gym 注册中心的环境注册。您现在可以使用 OpenAI Gym 来创建我们构建的定制环境的实例。

以下屏幕截图显示了可用于测试自定义Gym环境的 Python 命令:
...
在本章中，我们一步一步地介绍了自定义Gym环境的实现，从一个模板开始，该模板展示了 OpenAI Gym环境的基本结构，为智能体提供了所有必要的接口。我们还研究了如何在 Gym 注册表中注册一个定制环境实现，这样我们就可以使用熟悉的`gym.make(ENV_NAME)`命令来创建一个现有环境的实例。然后，我们看了如何为基于开源驾驶模拟器 CARLA 的非远程引擎创建一个Gym兼容的环境实现。然后，我们快速浏览了安装和运行 CARLA 所需的步骤，然后开始一部分一部分地实现`CarlaEnv`类，仔细地涵盖了实现与 OpenAI Gym 兼容的定制环境所涉及的所有重要细节。
