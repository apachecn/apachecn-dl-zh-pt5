...
在这一章中，我们将看看用来提高随机策略梯度方法稳定性的方法。已经进行了一些尝试来使策略改进更加稳定，我们将集中讨论三种方法:**最近策略优化** ( **PPO** )、**信任区域策略优化** ( **TRPO** )和**演员-评论家** ( **A2C** )使用**克罗内克因子信任区域** ( **ACKTR** )。
...
对经典的**异步优势演员-评论家** ( **A3C** )方法的核心改进是改变用于估算 PG 的表达式。PPO 方法没有采用所采取行动的对数概率梯度，而是采用了不同的目标:新旧策略之间的比率与优势成比例。
