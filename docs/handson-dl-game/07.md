...
# RL 已经变得如此流行，以至于现在出现了一场只构建工具来帮助构建 RL 算法的竞赛。目前这一领域的两个主要竞争对手是**开放Gym**和 **Unity** 。Unity 很快成为了 RL 赛车，我们将在后面深入探讨。现在，我们将戴上训练轮，运行 OpenAI Gym，进一步探索 RL 的基础。
...
2.  就我们的目的而言，在短期内，我们不需要安装任何其他的Gym模块。Gym 有大量的示例环境，Atari 游戏和 MuJoCo(机器人模拟器)是最有趣的工作环境。我们将在本章的后面看一下 Atari 游戏模块。
...
这将为您的系统安装Gym环境。我们需要的大部分东西只需很少的设置就能工作。如果你决定用 Gym 做更多的事情，那么你可能会想安装其他模块；有好几个。在下一节中，我们将在学习 Q-Learning 时测试这个新环境。
...
7.  尝试在来自`Chapter_5_5.py`的 DQNAgent 示例中使用不同的Gym环境。你可以在谷歌上快速搜索一下，看看有没有其他可供选择的环境。
8.  `Chapter_5_6.py`示例目前使用一种叫做`LinearAnnealedPolicy`的表单探索策略；将策略更改为使用代码注释中提到的`BoltzmannQPolicy`策略。
9.  请务必下载并运行来自 https://github.com/keras-rl/keras-rl[的其他 Keras-RL 示例。同样，您可能需要安装其他Gym环境来让它们工作。](https://github.com/keras-rl/keras-rl)
...
# RL 是目前主导许多研究人员兴趣的机器学习技术。它对我们很有吸引力，因为它非常适合游戏和模拟。在这一章中，我们从多兵种背景强盗的基本入门问题开始，讨论了 RL 的一些基础。然后，我们快速查看了安装 OpenAI Gym RL 工具包。然后我们看了 Q-learning 以及如何在代码中实现它，并在 OpenAI Gym环境中训练它。最后，我们研究了如何通过加载一些其他环境，包括 Atari 游戏模拟器，用 Gym 进行各种其他实验。
