...
使用 Python 来训练效果很好，但这不是一个真正的游戏会使用的东西。理想情况下，我们希望能够建立一个TensorFlow图，并在 Unity 中使用它。幸运的是，我们构建了一个名为 TensorFlowSharp 的库。NET 来消耗TensorFlow图。这允许我们建立离线的 TF 模型，然后将它们注入到我们的游戏中。不幸的是，我们只能使用经过训练的模型，而不能以这种方式训练，至少现在还不能。
