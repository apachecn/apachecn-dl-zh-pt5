...
2.  这个示例被配置为使用几个并发的培训环境。我们能够用同一个大脑训练多个并发的训练环境，因为**近似策略优化** ( **PPO** )，驱动这个代理的 RL 算法，训练到一个策略而不是一个模型。当我们在第 8 章、*了解 PPO、*学习 RL 中了解 PPO 的内部原理时，我们将涵盖策略和基于模型的学习的基础。出于我们的目的和简单起见，我们现在将禁用这些额外的环境。
