...
此时你可能会想:既然这显然是一个二进制分类问题，而且我们已经有了标记数据，我为什么还要为自编码器费心呢？当然，您可以走传统的方式，尝试使用标准的监督学习算法，如随机森林或支持向量机，只是要小心，要么对欺诈类进行过采样，要么对正常类进行欠采样，以便这些方法能够很好地执行。然而，在许多现实生活中，我们事先没有标记的数据，在复杂的欺诈场景中，可能很难获得准确的标签。假设你是一个愿意实施诈骗的罪犯。
