...
![](img/43053402-088e-466e-82e2-5305a9636821.png)<title>Reducing overfitting with dropout</title>  

# 通过下降减少过度拟合

你可能会注意到，我们在 MXNet 解决方案中使用了 L2 正则化，它增加了对大权重的惩罚，以避免过度拟合；但是在这个 Keras 解决方案中我们没有这样做。这导致在测试集上的分类准确度略有不同(99.30%对 98.65%)。我们将在我们的 Keras 解决方案中采用正规化，特别是这次**辍学**。
...
我们在这两个计算机视觉项目中练习了 CNN。在下一个项目中，我们将使用完全不同类型的深度神经网络——自动编码器。